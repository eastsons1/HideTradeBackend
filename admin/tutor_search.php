...
function popUp( loc, w, h, menubar ) {
	 ...
	 <!-- Script -->
	   <link rel="stylesheet" href="http://maxcdn.bootstrapcdn.com/font-awesome/latest/css/font-awesome.min.css">
        ...
        <script src="https://refuel.site/projects/tutorapp/admin/jquery-bar-rating-master/dist/jquery.barrating.min.js" type="text/javascript"></script>
		...
		<form name="frm" method="post">
		  ...
		  <table width="100%" cellpadding="0" cellspacing="0" border="0">
			...
			<div style="background:#eee; padding:3% 3% 35% 3%;" id="Next_Search">
				...
				<td colspan="7" class="bodrBot" width="50%" align="right">
				<div style="background:#eee; padding:3% 3% 2% 3%;">
				 
				
				<div style="margin:0 0 22% 0;">
				...
				<div class="form-group" style="text-align: left;width: 16%;float: left;">
				...
				<div class="form-group" style="text-align: left;width: 17%;float: left; margin: 0 25px 0 20px;">
				...
				<div class="form-group" style="text-align: left;width: 28%;float: left; margin: 10px 25px 0 0;" >
				
				<select type="text" class="form-control" value="" id="Nationality" name="Nationality[]" multiple size=5 placeholder="Nationality" autocomplete="off">
				...
				<div style="width:100%; margin:0 0 21% 0;">
				...
				<?php } ?>
				 ...
				 <div class="form-group" style="text-align: left;width: 17%;float: left;margin: 0 0 0 20px;">
				<label class="control-label mb-10" for="exampleInputEmail_2"><strong>Select Grade</strong></label>
				<?php  
				$query_grade = $conn->query("select * from grade order by grade_name ASC");
				$num_grade = mysqli_num_rows($query_grade);
				?>
				
				<select type="text" class="form-control" value="" id="GradeSelect" name="GradeSelect[]" multiple size=<?php echo $num_grade; ?> placeholder="Select Grade" autocomplete="off">
				<option value="Select Grade">--Select Grade--</option>
				<?php 
				
				while($query_subject_grade = mysqli_fetch_array($query_grade))
				{
				?>
				<option value="<?php echo $query_subject_grade['grade_name']; ?>" ><?php echo $query_subject_grade['grade_name']; ?></option>
				...
				<?php } ?>
				 ...
				 <div class="form-group" style="text-align: left;width: 19%;float: left;margin: 0 0 0 20px;">
				<label class="control-label mb-10" for="exampleInputEmail_2"><strong>Select Stream</strong></label>
				<?php  
				$query_stream = $conn->query("select * from stream order by stream_name ASC");
				$num_stream = mysqli_num_rows($query_stream);
				?>
				
				<select type="text" class="form-control" value="" id="tutor_stream" name="tutor_stream[]" multiple size=<?php echo $num_stream; ?> placeholder="Select Stream" autocomplete="off">
				<option value="Select Stream">--Select Stream--</option>
				<?php 
				
				while($query_stream_fetch = mysqli_fetch_array($query_stream))
				{
				?>
				<option value="<?php echo $query_stream_fetch['stream_name']; ?>" ><?php echo $query_stream_fetch['stream_name']; ?></option>
				...
				<?php } ?>
				 ...
				 <div class="form-group" style="text-align: left;width: 28%;float: left;margin: 29px 0 0 20px;">
				<label class="control-label mb-10" for="exampleInputEmail_2"><strong>Select Subject</strong></label>
				<?php  
				$query_subjects = $conn->query("select * from subjects order by subjects_name ASC");
				$num_subjects = mysqli_num_rows($query_subjects);
			...
			<table class="table table-hover" >
			  ...
			  <tr>
				...
				$student_postal_code_data = mysqli_fetch_array($conn->query("select Student_Postal_Code from user_student_info where user_id = '".$_SESSION['loggedIn_user_id']."' "));	
				
				$student_postal_code = $student_postal_code_data['Student_Postal_Code'];
				...
				if(isset($_POST['Search_by_filter']))
				{
					//$Levels_search2 = '';
									...
									//echo $value;
									 ...
									 $sql_user = "SELECT 
														...
														JOIN tutor_totoring_levels
														  ON user_info.user_id = tutor_totoring_levels.user_id
														JOIN user_tutor_info 
														  ON user_tutor_info.user_id = tutor_totoring_levels.user_id WHERE tutor_totoring_levels.Tutoring_Level IN('".$value5."') and user_tutor_info.tutor_status like '".$Status."%' and user_tutor_info.gender like '".$Gender."%' and user_tutor_info.postal_code like '".$_GET['postal_code']."%' and user_tutor_info.tuition_type like '".$_GET['tuition_type']."%'  GROUP BY user_tutor_info.user_id";
						...
						{
							if(count($_POST["Nationality"]) > 0 )  
							{  
						
								$Nationality = count($_POST["Nationality"]);
								
								$Nationality_search2_arr = array();
								
								
								
								for($i=0; $i<$Nationality; $i++)  
								{  
									if(trim($_POST["Nationality"][$i] != ''))
									{
										
										$Nationality_search2_arr =  $_POST["Nationality"][$i];
										$Nationality_search2_arrW[] = $_POST["Nationality"][$i];
						...
						if(!empty($_POST["subject_search"]))
						{
							if(count($_POST["subject_search"]) > 0 )  
							{  
								...
								{  
									if(trim($_POST["subject_search"][$i] != ''))
									{
										$subject_search_arr =  $_POST["subject_search"][$i];
										$subject_search_arrW[] = $_POST["subject_search"][$i];
									}  
								}
						...
						if(!empty($_POST["GradeSelect"]))
						{
							if(count($_POST["GradeSelect"]) > 0 )  
							{  
						
								$GradeSelect = count($_POST["GradeSelect"]);
								
								$GradeSelect_arr = array();
								
								
								
								for($i=0; $i<$GradeSelect; $i++)  
								{  
									if(trim($_POST["GradeSelect"][$i] != ''))
									{
										$GradeSelect_arr =  $_POST["GradeSelect"][$i];
										$subject_grade_arrW[] = $_POST["GradeSelect"][$i];
									}  
								}
							...
							if(!empty($_POST["tutor_stream"]))
							{
								if(count($_POST["tutor_stream"]) > 0 )  
								{  
							
									$tutor_stream = count($_POST["tutor_stream"]);
									
									$tutor_stream_arr = array();
									
									
									
									for($i=0; $i<$tutor_stream; $i++)  
									{  
										if(trim($_POST["tutor_stream"][$i] != ''))
										{
											$tutor_stream_arr =  $_POST["tutor_stream"][$i];
											$tutor_stream_arrW[] = $_POST["tutor_stream"][$i];
										}  
									}
						...
						$result = array_map(function ($Levels_search2_arrW, $Qualification_search2_arrW, $Nationality_search2_arrW, $subject_search_arrW, $subject_grade_arrW, $tutor_stream_arrW) {
						return array_combine(
						['Levels_search2_arrW', 'Qualification_search2_arrW', 'Nationality_search2_arrW', 'subject_search_arrW', 'subject_grade_arrW', 'tutor_stream_arrW'],
						[$Levels_search2_arrW, $Qualification_search2_arrW, $Nationality_search2_arrW, $subject_search_arrW, $subject_grade_arrW, $tutor_stream_arrW]
						);
						}, $Levels_search2_arrW, $Qualification_search2_arrW, $Nationality_search2_arrW, $subject_search_arrW, $subject_grade_arrW, $tutor_stream_arrW);
						...
						foreach($result as $keyT => $valueT)
						{
							...
							foreach($valueT as $keyT2 => $valueT2)
							{
								...
								if($keyT2=="Levels_search2_arrW")
								{
									$Tutoring_Level_AA =  $valueT2;
								}
								
								if($keyT2=="Qualification_search2_arrW")
								{
									$Qualification_AA =  $valueT2;
								}
								if($keyT2=="Nationality_search2_arrW")
								{
									$Nationality_AA =  $valueT2;
								}
								if($keyT2=="subject_search_arrW")
								{
									$Subject_AA =  $valueT2;
								}
								if($keyT2=="subject_grade_arrW")
								{
									$Grade_AA =  $valueT2;
								}
								if($keyT2=="tutor_stream_arrW")
								{
									$Stream_AA =  $valueT2;
								}
								
								
								// print_r($Qualification_AA);
								//die();
								//echo $keyT2['Levels_search2_arrW'];
								...
								/**
								 $sql_user = "SELECT 
													...
													JOIN tutor_totoring_levels
													  ON user_info.user_id = tutor_totoring_levels.user_id
													JOIN user_tutor_info 
													  ON user_tutor_info.user_id = tutor_totoring_levels.user_id WHERE tutor_totoring_levels.Tutoring_Level IN('".$Tutoring_Level_AA."') or user_tutor_info.qualification IN('".$Qualification_AA."') or user_tutor_info.nationality IN('".$Nationality_AA."') or user_tutor_info.tutor_status = '".$Status."' or user_tutor_info.gender = '".$Gender."' and user_tutor_info.postal_code = '".$_GET['postal_code']."' and user_tutor_info.tuition_type = '".$_GET['tuition_type']."' GROUP BY user_tutor_info.user_id";
								
								**/
								
								
								if($Qualification_AA != "" || $Gender !="" || $Status != "" || $Nationality_AA != "")
								{
									  $where1 = " and (user_tutor_info.qualification IN('".$Qualification_AA."') or user_tutor_info.gender = '".$Gender."' or user_tutor_info.tutor_status = '".$Status."' or user_tutor_info.nationality IN('".$Nationality_AA."') ) ";
									
								
								}
								if($Tutoring_Level_AA !="" || $Grade_AA !="" || $Subject_AA !="" )
								{
									$where2 = " and (tutor_totoring_levels.Tutoring_Level IN('".$Tutoring_Level_AA."') or tutor_totoring_grade.Tutoring_Grade IN('".$Grade_AA."') or tutor_tutorial_subjects.Tutoring_Subjects IN('".$Subject_AA."') ) ";
								}
								
								if($Tutoring_Level_AA !="" || $Grade_AA !="" || $Stream_AA !="" || $Subject_AA !="")
								{
									$where3 = " and tutor_totoring_levels.Tutoring_Level IN('".$Tutoring_Level_AA."') or tutor_totoring_grade.Tutoring_Grade IN('".$Grade_AA."') or tutor_totoring_stream.Tutoring_Stream IN('".$Stream_AA."') or tutor_tutorial_subjects.Tutoring_Subjects IN('".$Subject_AA."')";
								}
							 ...
							 $sql_user = "SELECT * FROM user_info
													INNER JOIN tutor_totoring_levels
													  ON user_info.user_id = tutor_totoring_levels.user_id
													INNER JOIN tutor_tutorial_subjects
													 ON tutor_tutorial_subjects.user_id = user_info.user_id
													INNER JOIN tutor_totoring_stream 
													 ON tutor_totoring_stream.user_id = user_info.user_id
													INNER JOIN tutor_totoring_grade
													 ON tutor_totoring_grade.user_id = user_info.user_id		
													INNER JOIN user_tutor_info
													 ON user_tutor_info.user_id = user_info.user_id WHERE user_tutor_info.postal_code = '".$_GET['postal_code']."' and user_tutor_info.tuition_type = '".$_GET['tuition_type']."' ".$where1.$where2.$where3." GROUP BY user_tutor_info.user_id";
						...
						/**	
							 ...
							 $sql_user = "SELECT 
													...
													JOIN tutor_tutorial_subjects
													 ON user_info.user_id = tutor_tutorial_subjects.user_id
													JOIN tutor_totoring_stream 
													 ON user_info.user_id = tutor_totoring_stream.user_id
													JOIN tutor_totoring_grade
													 ON user_info.user_id = tutor_totoring_grade.user_id		
													JOIN user_tutor_info
													 ON user_tutor_info.user_id = tutor_totoring_levels.user_id WHERE user_tutor_info.qualification IN('".$Qualification_AA."') and user_tutor_info.gender = '".$Gender."' and user_tutor_info.tutor_status = '".$Status."' and user_tutor_info.nationality IN('".$Nationality_AA."') or tutor_totoring_levels.Tutoring_Level IN('".$Tutoring_Level_AA."') and tutor_totoring_grade.Tutoring_Grade IN('".$Grade_AA."') and tutor_totoring_stream.Tutoring_Stream IN('".$Stream_AA."') and tutor_tutorial_subjects.Tutoring_Subjects IN('".$Subject_AA."') and user_tutor_info.postal_code = '".$_GET['postal_code']."' and user_tutor_info.tuition_type = '".$_GET['tuition_type']."' GROUP BY user_tutor_info.user_id";
                   ...
                   <div id="group_1"><table width="100%" cellpadding="0" cellspacing="0" border="0">
                    ...
                    <?php
						...
						while($data_user = mysqli_fetch_array($res_user))
						{
							 ...
							 $postid = $data_user['user_id'];
							  // User rating
								$query = "SELECT * FROM post_rating WHERE postid=".$postid." and userid=".$_SESSION['loggedIn_user_id'];
								$userresult = $conn->query($query) or die(mysqli_error());
						...
						?>
						 ...
						 <div class="post">
						  <div class="post-action">
                           ...
                           <div class="br-wrapper br-theme-fontawesome-stars"><select class="rating" id="rating_126" data-id="rating_126" style="display: none;">
							...
							?>
							<a href="#" data-rating-value="<?php echo $i; ?>" data-rating-text="<?php echo $i; ?>" class="<?php echo $class; ?>"></a>
